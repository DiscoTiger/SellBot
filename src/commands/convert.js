...
class Convert extends Command {
	...
	run(msg, args) {
		...
		if (symbol2) {
			...
			let numOther = (ammount * this.client.tickers.get(symbol1).price_btc) / this.client.tickers.get(symbol2).price_btc; // eslint-disable-line max-len
			msg.channel.send(`${numOther} ${symbol2}`);
		} else {
			msg.channel.send(`$${this.client.tickers.get(symbol1).price_usd * ammount}`);
		}
	}
}
